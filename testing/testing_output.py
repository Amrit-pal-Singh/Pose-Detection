...
if __name__ == '__main__':
    ...
    if(args.file_output):
        out = cv2.VideoWriter(args.file_output,fourcc, 20.0, (int(cam.get(3)), int(cam.get(4))))    
    else:
    ...
    while(cam.isOpened()):
        ...
        if(count == 3):
            ...
            for human in humans:
                ...
                dis = get_distance.compute_distance(x_y_body_parts, 'eul', 'list')
                mean_file = open('mean.txt')
                std_file = open('std.txt')
                means = mean_file.readline().rstrip().split(' ')
                print(means)
                stds = std_file.readline().rstrip().split(' ')
                print(stds)
                if(dis != -1 and len(dis) == len(means)):
                    for i in range(len(means)):
                        dis[i] = (dis[i] - float(means[i]))/float(stds[i])
                    list_body = np.asarray(dis, dtype=float)
                    li = [list_body]
                    li = np.asarray( np.asarray(li) )
                    model_new = load_model('98_1.h5')
